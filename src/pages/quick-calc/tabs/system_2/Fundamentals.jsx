...
const ChartLink = ({ href }) => {
  ...
  const btnStyle = { fontSize: 10, border: '1px solid #adc6ff', borderRadius: 4, padding: '2px 7px', textDecoration: 'none', flexShrink: 0, cursor: 'pointer', background: '#fff' };
  return (
    <div style={{ display: 'flex', gap: 4, flexShrink: 0 }}>
      <a href={href} target="_blank" rel="noreferrer" style={{ ...btnStyle, color: '#1677ff' }}>↗ 图表</a>
      <button onClick={handleCopy} style={{ ...btnStyle, color: copied ? '#389e0d' : '#8c8c8c', borderColor: copied ? '#b7eb8f' : '#adc6ff' }}>
        {copied ? '✓ 已复制' : '复制链接'}
      </button>
    </div>
  );
...
const Section = ({ title, children }) => (
  <div style={{ marginBottom: 24 }}>
    <div style={{
      ...
      borderLeft: '3px solid #1677ff', paddingLeft: 10, marginBottom: 12,
    }}>
      {title}
    </div>
    {children}
...
const Block = ({ label, children, color = '#f5f5f5', borderColor = '#d9d9d9' }) => (
  <div style={{
    ...
    {label && (
      <div style={{ fontSize: 11, fontWeight: 600, color: '#8c8c8c', marginBottom: 6, textTransform: 'uppercase', letterSpacing: 0.5 }}>
        {label}
      </div>
    )}
...
const Pending = ({ label }) => (
  <div style={{
    ...
    borderRadius: 6, padding: '10px 14px', marginBottom: 10,
    fontSize: 12, color: '#bfbfbf', fontStyle: 'italic',
  }}>
    {label}（待补充）
  </div>
);

const FundamentalCheck = ({ num, question, answer, links, embed, color = '#1677ff', bg = '#e6f7ff', border = '#91d5ff' }) => (
  <div style={{
    ...
    <div style={{ fontSize: 13, color: '#434343', lineHeight: 1.7, paddingLeft: 36 }}>
      {answer}
    </div>
    {links && (
...
const CounterQuestion = ({ scenario, result }) => (
  <div style={{
    ...
    fontSize: 13, color: '#434343', lineHeight: 1.7,
  }}>
  ...
  <div style={{ padding: '4px 0' }}>
    ...
    <Section title="怎么做：两步走">
      <Block label="第一层：为什么有人需要它" color="#f6ffed" borderColor="#b7eb8f">
        ...
        <div style={{ marginTop: 8 }}>
          ...
          这就是它的核心主张——不是总量 2100 万这个数字，而是「固定上限 + 不依赖信任」解决了什么问题。
        </div>
    ...
    <Section title="反面问：什么事发生 = 不成立">
      ...
      <Block label="反面问怎么写" color="#e6f7ff" borderColor="#91d5ff">
        ...
        <div style={{ marginTop: 4 }}>写的标准：具体到可以验证。不能是「市场不认可了」这种无法证伪的说法。</div>
      </Block>
    </Section>
    ...
    <Section title="为什么简单是必须的">
      <Block color="#f0f5ff" borderColor="#adc6ff">
        ...
        <div style={{ marginTop: 6 }}>
          黄金值钱不是因为增长率，是因为全世界都认可它作为价值储藏。
          BTC 的逻辑一样：固定上限 + 全球可转移。
        </div>
        <div style={{ marginTop: 6, color: '#8c8c8c', fontSize: 12 }}>
  ...
  <div style={{ padding: '4px 0' }}>
    <Section title="核心主张（人们为什么需要 BTC）">
      ...
      <div style={{
        background: '#fff1f0', border: '1px solid #ffa39e',
        borderRadius: 6, padding: '10px 14px', marginBottom: 10,
        fontSize: 12, color: '#a8071a', lineHeight: 1.7,
      }}>
    ...
    <Section title="支撑主张的底层条件（这些还在吗？）">
      ...
      <FundamentalCheck
        ...
        links={[
          ...
          { label: '算力 & 难度 mempool.space', url: 'https://mempool.space/graphs/mining/hashrate-difficulty' },
      ...
      <FundamentalCheck
        ...
        links={[
          { label: '网络实时状态 mempool.space', url: 'https://mempool.space/' },
        ]}
      />
      <FundamentalCheck
        num={4}
        ...
        links={[
          { label: 'ETF 资金流向对比', url: 'https://farside.co.uk/bitcoin-etf-flow-all-data-chart/' },
      ...
      <div style={{ marginTop: 4, marginBottom: 4 }}>
        {[
          ...
          <div key={i} style={{
            ...
            fontSize: 12, color: '#a8071a', display: 'flex', gap: 8, alignItems: 'flex-start',
          }}>
    ...
    <Section title="入场时机指标">
      <div style={{ fontSize: 12, color: '#8c8c8c', marginBottom: 16 }}>
        四个指标来自两个独立维度——矿工行为 和 持有者成本——同时触发说明两个不同群体都在极度痛苦，交叉验证比单维度更可靠。
      </div>

      {/* ── 最强信号组 ── */}
      <div style={{ marginBottom: 24 }}>
        <div style={{ background: '#f6ffed', border: '2px solid #52c41a', borderRadius: 8, padding: '12px 16px', marginBottom: 10 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
            <span style={{ fontSize: 11, fontWeight: 700, color: '#fff', background: '#52c41a', borderRadius: 4, padding: '2px 8px' }}>最强信号</span>
            <span style={{ fontSize: 13, fontWeight: 700, color: '#135200' }}>Hash Ribbons 金叉 + MVRV Z-Score 进入绿区</span>
          </div>
          <div style={{ fontSize: 12, color: '#237804', lineHeight: 1.7 }}>
            矿工投降结束（抛压枯竭）× 整体市场低于链上成本（极端恐慌）。两个完全独立的维度同时确认，历史上 2015、2018-19、2022 年三次大底全部命中。两个都触发才算，单独一个不够。
          </div>
        </div>

        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
          {/* Hash Ribbons */}
          <div style={{ background: '#fafafa', border: '1px solid #e8e8e8', borderRadius: 8, padding: '12px 14px' }}>
              ...
              <div>
                <div style={{ fontSize: 13, fontWeight: 700, color: '#262626' }}>
                  ...
                  <Tooltip title="由两条算力移动均线（30天MA 和 60天MA）构成。当短期算力跌破长期算力，说明矿工大量关机（亏本了）；当短期重新穿越长期向上，说明最弱的矿工已经出清，剩下的是低成本生存者——市场最艰难的阶段结束了。">
                    <InfoCircleOutlined style={{ fontSize: 11, color: '#8c8c8c', marginLeft: 5, cursor: 'pointer' }} />
                  </Tooltip>
                </div>
                <div style={{ fontSize: 11, color: '#8c8c8c', marginTop: 2 }}>算力带：矿工投降结束信号</div>
              </div>
              <ChartLink href="https://www.lookintobitcoin.com/charts/hash-ribbons/" />
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginBottom: 8 }}>
              <div style={{ background: '#fff1f0', border: '1px solid #ffa39e', borderRadius: 5, padding: '6px 10px', fontSize: 11 }}>
                <b style={{ color: '#cf1322' }}>死叉</b>：30日MA 跌破 60日MA → 矿工关机，抛压加大
              </div>
              <div style={{ background: '#f6ffed', border: '1px solid #b7eb8f', borderRadius: 5, padding: '6px 10px', fontSize: 11 }}>
                <b style={{ color: '#389e0d' }}>金叉 ✓</b>：30日MA 上穿 60日MA → 投降结束，底部形成
              </div>
            </div>
            <iframe src="https://www.lookintobitcoin.com/charts/hash-ribbons/"
              width="100%" height="220" style={{ border: 'none', borderRadius: 6, display: 'block' }} title="Hash Ribbons" />
          </div>
          ...
          <div style={{ background: '#fafafa', border: '1px solid #e8e8e8', borderRadius: 8, padding: '12px 14px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
              <div>
                ...
                <div style={{ fontSize: 11, color: '#8c8c8c', marginTop: 2 }}>市值 vs 链上成本：市场极度低估信号</div>
              </div>
              <ChartLink href="https://www.lookintobitcoin.com/charts/mvrv-zscore/" />
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginBottom: 8 }}>
              <div style={{ background: '#f6ffed', border: '1px solid #b7eb8f', borderRadius: 5, padding: '6px 10px', fontSize: 11 }}>
                <b style={{ color: '#389e0d' }}>绿区（&lt; 0）✓</b>：大多数人亏损，历史每次大底都在此
              </div>
              <div style={{ background: '#fff1f0', border: '1px solid #ffa39e', borderRadius: 5, padding: '6px 10px', fontSize: 11 }}>
                <b style={{ color: '#cf1322' }}>红区（&gt; 7）</b>：牛市狂热顶部，历史每次顶部都在此
              </div>
            </div>
            <iframe src="https://www.lookintobitcoin.com/charts/mvrv-zscore/"
              width="100%" height="220" style={{ border: 'none', borderRadius: 6, display: 'block' }} title="MVRV Z-Score" />
          </div>
        </div>
      </div>
      ...
      <div style={{ marginBottom: 16 }}>
        <div style={{ background: '#e6f7ff', border: '2px solid #1677ff', borderRadius: 8, padding: '12px 16px', marginBottom: 10 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
            <span style={{ fontSize: 11, fontWeight: 700, color: '#fff', background: '#1677ff', borderRadius: 4, padding: '2px 8px' }}>强信号</span>
            <span style={{ fontSize: 13, fontWeight: 700, color: '#003a8c' }}>Puell Multiple &lt; 0.5 + 价格在 Realized Price 附近或以下</span>
          </div>
          <div style={{ fontSize: 12, color: '#0050b3', lineHeight: 1.7 }}>
            矿工收入极低（接近关机临界）× 全网平均买入成本附近——两个成本锚点同时显示低估。不必等最强信号（最强信号往往在恐慌最深处才出现，等到时候可能已错过起涨段），适合开始分批建仓。
          </div>
        </div>
          ...
          <div style={{ background: '#fafafa', border: '1px solid #e8e8e8', borderRadius: 8, padding: '12px 14px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
              <div>
                ...
                <div style={{ fontSize: 11, color: '#8c8c8c', marginTop: 2 }}>矿工收入倍数：量化矿工盈利压力</div>
              </div>
              <ChartLink href="https://www.lookintobitcoin.com/charts/puell-multiple/" />
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginBottom: 8 }}>
              <div style={{ background: '#f6ffed', border: '1px solid #b7eb8f', borderRadius: 5, padding: '6px 10px', fontSize: 11 }}>
                <b style={{ color: '#389e0d' }}>&lt; 0.5 ✓</b>：矿工极度亏损，历史级别买入区
              </div>
              <div style={{ background: '#e6f7ff', border: '1px solid #91d5ff', borderRadius: 5, padding: '6px 10px', fontSize: 11 }}>
            ...
            <iframe src="https://www.lookintobitcoin.com/charts/puell-multiple/"
              width="100%" height="220" style={{ border: 'none', borderRadius: 6, display: 'block' }} title="Puell Multiple" />
          </div>
          ...
          <div style={{ background: '#fafafa', border: '1px solid #e8e8e8', borderRadius: 8, padding: '12px 14px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
              <div>
                ...
                <div style={{ fontSize: 11, color: '#8c8c8c', marginTop: 2 }}>全网平均买入成本线</div>
              </div>
              <ChartLink href="https://studio.glassnode.com/charts/market.PriceRealizedUsd?a=BTC" />
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginBottom: 8 }}>
              <div style={{ background: '#f6ffed', border: '1px solid #b7eb8f', borderRadius: 5, padding: '6px 10px', fontSize: 11 }}>
            ...
            <iframe src="https://studio.glassnode.com/charts/market.PriceRealizedUsd?a=BTC"
              width="100%" height="220" style={{ border: 'none', borderRadius: 6, display: 'block' }} title="Realized Price" />
          </div>
        </div>
      </div>

      <div style={{ background: '#fffbe6', border: '1px solid #ffe58f', borderRadius: 6, padding: '10px 14px', fontSize: 12, color: '#7c4a15', lineHeight: 1.7 }}>
  ...
  <div style={{ padding: '4px 0' }}>
    ...
    <Section title="核心主张（人们为什么需要 ETH）">
      ...
      <div style={{
        ...
        padding: '10px 14px', fontSize: 12, color: '#874d00', lineHeight: 1.7,
      }}>
        ⚠ ETH 和 BTC 的本质区别：BTC 的价值不依赖使用量（稀缺性本身有价值），
        ETH 的价值直接依赖使用量——没人用以太坊，ETH 就是没人要的燃料。
      </div>
    </Section>
    ...
    <Section title="需求来源（谁在用、为什么用）">
      ...
      <Block label="需求 2：稳定币结算层" color="#f6ffed" borderColor="#b7eb8f">
        全球最大的稳定币（USDC、USDT）大量在以太坊上发行和流通。
        每一笔稳定币转账都需要消耗 ETH 作为 Gas 费。
      </Block>
      ...
      <Block label="需求 4：Layer 2 结算层" color="#f6ffed" borderColor="#b7eb8f">
        Arbitrum、Optimism、Base 等 L2 链把大量交易批量压缩后提交到以太坊主链做最终确认。
        L2 越活跃，以太坊收的结算费越多，ETH 消耗越多。
      </Block>
    </Section>
    ...
    <Section title="支撑主张的底层条件（这些还在吗？）">
      ...
      <FundamentalCheck
        ...
        color="#627eea" bg="#f0f5ff" border="#adc6ff"
        links={[
          { label: '节点分布 ethernodes.org', url: 'https://ethernodes.org/' },
        ]}
      />
      <FundamentalCheck
        num={2}
        ...
        color="#627eea" bg="#f0f5ff" border="#adc6ff"
        links={[
          { label: 'ETH 实时销毁 ultrasound.money', url: 'https://ultrasound.money/' },
        ]}
      />
      <FundamentalCheck
        num={3}
    ...
    <Section title="ETH 的发行机制">
      <Block color="#f0f5ff" borderColor="#adc6ff">
        <div style={{ fontWeight: 600, marginBottom: 8 }}>ETH 没有固定总量上限，供应量由两个力量对冲：</div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 12 }}>
          <div style={{ background: '#fff1f0', border: '1px solid #ffa39e', borderRadius: 6, padding: '10px 12px' }}>
            <div style={{ fontWeight: 700, color: '#cf1322', marginBottom: 4, fontSize: 12 }}>增发 ↑</div>
            <div style={{ fontSize: 12, lineHeight: 1.7 }}>PoS 验证节点奖励：质押 32 ETH 成为验证者，每年获得约 3-4% ETH 奖励。这是 ETH 新增供应的唯一来源。</div>
          </div>
          <div style={{ background: '#f6ffed', border: '1px solid #b7eb8f', borderRadius: 6, padding: '10px 12px' }}>
            <div style={{ fontWeight: 700, color: '#389e0d', marginBottom: 4, fontSize: 12 }}>销毁 ↓</div>
            <div style={{ fontSize: 12, lineHeight: 1.7 }}>EIP-1559 基础费销毁：每笔交易的基础手续费直接从供应量里销毁，不进任何人口袋。网络越忙，销毁越多。</div>
          </div>
        </div>
        <div style={{ background: '#e6f7ff', border: '1px solid #91d5ff', borderRadius: 6, padding: '10px 12px', fontSize: 12, lineHeight: 1.7 }}>
          <b>净效果</b>：网络忙 → 销毁 &gt; 增发 → ETH 通缩（变稀缺）<br />
          网络闲 → 增发 &gt; 销毁 → ETH 通胀（变多）<br />
          <span style={{ color: '#8c8c8c' }}>与 BTC 的核心区别：BTC 稀缺性由协议固定，ETH 稀缺性由使用量决定。</span>
        </div>
      </Block>
      <div style={{ fontSize: 11, color: '#8c8c8c', marginTop: 4 }}>
        普通持有者不会自动获得新增发的 ETH，只有质押的验证者才有。想参与质押但没有 32 ETH，可以通过 Lido 等流动性质押协议以任意数量参与。
      </div>
    </Section>
    ...
    <Section title="怎么衡量 ETH 的价值">
      ...
      {[
        {
          rank: 1, label: '稳定币流通量', desc: 'USDC/USDT 在以太坊上的总量。稳定币每笔转账都消耗 ETH，是最稳定的真实需求来源，最难被投机炒作虚增。',
          tooltip: '最难造假：稳定币存在本身就产生持续的转账需求，不受市场情绪影响。即使在熊市，企业和个人仍然需要用稳定币结算，反映的是以太坊作为结算层的真实渗透程度。',
          link: { text: 'defillama stablecoins', url: 'https://defillama.com/stablecoins' },
        },
        {
          rank: 1, label: '机构资产上链（RWA）', desc: '贝莱德等机构把国债、基金代币化到以太坊的规模。增长代表机构级信任在加深，是最重要的长期采用信号。',
          tooltip: '与稳定币并列第一：机构把真实法律资产放到链上，背后有合规、审计、法律成本——这种信任不会因为市场情绪波动就消失，是结构性采用而非投机行为。',
          link: { text: 'rwa.xyz', url: 'https://rwa.xyz/' },
        },
        {
          rank: 3, label: '链上资产规模（TVL）', desc: '锁定在以太坊 DeFi 协议中的总资产，代表有多少资本信任以太坊保管资金。',
          tooltip: '排第三：TVL 是中期信心指标，但有一个缺陷——流动性挖矿会虚增数字（高收益率吸引资本进来，但一旦补贴停止就会撤走）。需要结合是否有真实使用场景来判断。',
          link: { text: 'defillama.com/chains', url: 'https://defillama.com/chains' },
        },
        {
          rank: 4, label: '网络使用量 → ETH 是否净通缩', desc: '销毁量 > 增发量 = ETH 在变稀缺，说明当前网络使用旺盛。',
          tooltip: '排第四：销毁量最敏感，也最嘈杂——投机交易同样能推高这个数字。牛市里大家疯狂交易，销毁量很高，但这不代表结构性采用，只能作为辅助信号，不能单独使用。',
          link: { text: 'ultrasound.money', url: 'https://ultrasound.money/' },
        },
        {
          rank: 5, label: 'L2 生态健康度', desc: 'L2 越活跃，向以太坊主链支付的结算费越多，ETH 消耗越多。',
          tooltip: '排最后：L2 活跃不直接等于 ETH 价值增长——L2 的目标是降低成本，成功的 L2 反而会减少主链手续费。它更像是生态健康的间接指标，而非直接的 ETH 需求驱动力。',
        ...
        <div key={i} style={{
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 8 }}>
            ...
            <a href={item.link.url} target="_blank" rel="noreferrer" style={{
              fontSize: 11, color: '#627eea', textDecoration: 'none',
              border: '1px solid #adc6ff', borderRadius: 4, padding: '2px 8px', flexShrink: 0,
            }}>↗ {item.link.text}</a>
          </div>
    ...
    <Section title="入场时机指标">
      <div style={{ background: '#fffbe6', border: '1px solid #ffe58f', borderRadius: 6, padding: '10px 14px', fontSize: 12, color: '#7c4a15', lineHeight: 1.7, marginBottom: 16 }}>
        ⚠ ETH 于 2022 年才完成从 PoW 到 PoS 的切换，历史上只经历过 2–3 次完整周期。以下信号的可信度分两档标注：已有历史数据支撑的放主区；逻辑合理但样本不足的单独标为「观察中」。
      </div>
      ...
      <div style={{ marginBottom: 24 }}>
        <div style={{ background: '#f6ffed', border: '2px solid #52c41a', borderRadius: 8, padding: '12px 16px', marginBottom: 10 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
            <span style={{ fontSize: 11, fontWeight: 700, color: '#fff', background: '#52c41a', borderRadius: 4, padding: '2px 8px' }}>有历史支撑</span>
            <span style={{ fontSize: 13, fontWeight: 700, color: '#135200' }}>ETH MVRV Z-Score 进绿区 + ETH 价格在 Realized Price 附近或以下</span>
          </div>
          <div style={{ fontSize: 12, color: '#237804', lineHeight: 1.7 }}>
            与 BTC 同一逻辑：持有者成本维度。2022 年 11 月（FTX 崩盘后约 $880）两个指标同时触发，是 ETH 迄今最清晰的底部数据点。样本只有 2–3 次，但逻辑自洽，是目前最可信的 ETH 入场信号。
          </div>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
          ...
          <div style={{ background: '#fafafa', border: '1px solid #e8e8e8', borderRadius: 8, padding: '12px 14px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
              <div>
                ...
                <div style={{ fontSize: 11, color: '#8c8c8c', marginTop: 2 }}>持有者成本：市场是否极度低估</div>
              </div>
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              {[
                { label: 'Glassnode：ETH MVRV Z-Score', url: 'https://studio.glassnode.com/charts/market.MvrvZScore?a=ETH', desc: '需要免费注册' },
                { label: 'Glassnode：ETH MVRV + Realized Price 合并图', url: 'https://studio.glassnode.com/charts/realizedprice-mvrv?a=ETH', desc: '两个指标放在一张图里，更直观' },
              ].map((l, i) => (
                <a key={i} href={l.url} target="_blank" rel="noreferrer" style={{
                  display: 'flex', justifyContent: 'space-between', alignItems: 'center',
                  background: '#f0f5ff', border: '1px solid #adc6ff', borderRadius: 5,
                  padding: '7px 10px', textDecoration: 'none',
                }}>
                  <span style={{ fontSize: 11, fontWeight: 600, color: '#1677ff' }}>↗ {l.label}</span>
          ...
          <div style={{ background: '#fafafa', border: '1px solid #e8e8e8', borderRadius: 8, padding: '12px 14px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              {[
                { label: 'Glassnode：ETH Realized Price', url: 'https://studio.glassnode.com/charts/market.PriceRealizedUsd?a=ETH', desc: '需要免费注册' },
                { label: 'Glassnode：ETH MVRV + Realized Price 合并图', url: 'https://studio.glassnode.com/charts/realizedprice-mvrv?a=ETH', desc: '两个指标放在一张图里，更直观' },
              ].map((l, i) => (
                <a key={i} href={l.url} target="_blank" rel="noreferrer" style={{
                  display: 'flex', justifyContent: 'space-between', alignItems: 'center',
      ...
      {/* ── 观察中（逻辑合理，样本不足） ── */}
      <div style={{ marginBottom: 16 }}>
        <div style={{ background: '#fff7e6', border: '2px solid #fa8c16', borderRadius: 8, padding: '12px 16px', marginBottom: 10 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
            <span style={{ fontSize: 11, fontWeight: 700, color: '#fff', background: '#fa8c16', borderRadius: 4, padding: '2px 8px' }}>观察中</span>
            <span style={{ fontSize: 13, fontWeight: 700, color: '#612500' }}>以下信号：逻辑合理，但缺乏足够历史验证</span>
          </div>
          <div style={{ fontSize: 12, color: '#873800', lineHeight: 1.7 }}>
            ETH 历史太短、结构变化太多（PoS 切换于 2022 年、EIP-1559 于 2021 年），下列信号还没有经历过足够多的完整周期来证明可靠性。记录在这里供参考，不与已验证信号等权重使用。
          </div>
        </div>
        ...
        {[
          {
            ...
            tooltip: '相当于平台股票的 P/S（价格/营收）极低：锁定在以太坊上的资产规模相对于 ETH 市值越大，说明平台越便宜。逻辑清晰，但 DeFi TVL 在 2018-19 底部时代基本不存在，无法跨周期验证。',
            why: '逻辑：平台使用量相对价格高 = 低估。缺陷：DeFi 2020 年才爆发，历史数据不够完整',
            link: { text: 'defillama.com/chains', url: 'https://defillama.com/chains' },
          },
          {
            label: '质押率高且无大规模解质押退出队列',
            tooltip: '大量 ETH 锁在 PoS 验证节点 = 长期持有者没有放弃，流动供应减少。逻辑类似 BTC 矿工还在挖矿（没有投降），但 PoS 质押机制 2022 年才开始，只有一个完整熊市周期的数据。',
            why: '逻辑：长期持有者仍在锁仓 = 没有系统性放弃。缺陷：只有 2022 年后数据，样本极少',
            link: { text: 'beaconcha.in', url: 'https://beaconcha.in/' },
          },
        ].map((item, i) => (
          <div key={i} style={{ background: '#fafafa', border: '1px solid #ffd591', borderRadius: 8, padding: '12px 14px', marginBottom: 10 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
              ...
              <a href={item.link.url} target="_blank" rel="noreferrer"
                style={{ fontSize: 10, color: '#d46b08', border: '1px solid #ffd591', borderRadius: 4, padding: '2px 7px', textDecoration: 'none', flexShrink: 0 }}>
                ↗ {item.link.text}
              </a>
            </div>
            <div style={{ fontSize: 11, color: '#873800', lineHeight: 1.6, background: '#fffbe6', borderRadius: 4, padding: '6px 10px' }}>
              {item.why}
            </div>
          </div>
        ))}
      ...
      <div style={{ background: '#f5f5f5', border: '1px solid #d9d9d9', borderRadius: 6, padding: '10px 14px', fontSize: 11, color: '#595959', lineHeight: 1.7 }}>
        ETH 入场逻辑和 BTC 的核心差异：BTC 有矿工关机价作为客观成本地板，ETH 没有等价物。ETH 的「地板」更模糊，持有者成本（MVRV + Realized Price）是目前唯一经过验证的锚点，但样本仍然偏少。保持更高的不确定性认知，仓位上比 BTC 更谨慎是合理的。
      </div>
    </Section>
...
const Fundamentals = ({ location }) => {
  ...
  return (
    <div>
      <div style={{
        display: 'flex', borderBottom: '1px solid #f0f0f0', marginBottom: 20,
      }}>
        {TABS.map(tab => (
          <div
            key={tab.key}
            ...
            style={{
              ...
              fontSize: 13, fontWeight: activeTab.key === tab.key ? 600 : 400,
              color: activeTab.key === tab.key ? '#1677ff' : '#595959',
              borderBottom: activeTab.key === tab.key ? '2px solid #1677ff' : '2px solid transparent',
              marginBottom: -1,
