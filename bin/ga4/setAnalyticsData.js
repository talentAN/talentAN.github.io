...
const setBlobPageData = rows => {
  ...
  const path_blog_data = paths[mode].path_blog_data;
  const totalCounts = JSON.parse(fs.readFileSync(path_blog_data, 'utf8'));
  ...
  fs.writeFileSync(path_blog_data, JSON.stringify(totalCounts));
